...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'settings', 'config'), s => {
      ...
      const d = s.data();
      // ✅ FIX: Firestore থেকে সরাসরি নেই — botUsername prop এর উপর নির্ভর নয়
      const rBot = (d.referralBotUsername || d.botUsername || '').replace('@','').trim();
      const rApp = (d.referralAppName || d.appName || '').replace('/','').trim();
      setCs({
    ...
    return () => unsub();
  }, []); // ✅ dependency array empty — একবারই setup হবে, Firestore realtime update দেবে
